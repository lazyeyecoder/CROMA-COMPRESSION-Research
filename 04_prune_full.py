...
def main():
    ...
    torch.save(croma.state_dict(), "croma_pruned_20_full.pt")   # distinct filename, don't overwrite subset's croma_pruned_20.pt
    ...
    torch.set_num_threads(16)   # use the 701 PC's cores for the eval loop
    val_dl = DataLoader(DFCDataset("validation", n=None), batch_size=16, shuffle=False)   # n=None = full 8,874 val images
    inter = torch.zeros(NUM_CLASSES); union = torch.zeros(NUM_CLASSES)
    print("Evaluating pruned model on FULL val set...")
    with torch.no_grad():
    ...
    present_classes = union > 0   # corrected formula, matches full-dataset baseline/quant runs
    ...
    print(f"Soft-pruned 20% mIoU (FULL): {miou:.4f}")

    print("Measuring latency (single-thread, matches laptop protocol)...")
    ...
    size_mb = os.path.getsize("croma_pruned_20_full.pt") / (1024*1024)

    print(f"\n--- SOFT-PRUNED 20% FULL-DATASET RESULTS ---")
    print(f"mIoU: {miou:.4f}  |  Latency: {latency_ms:.2f} ms/img  |  Size: {size_mb:.2f} MB")

    with open("results_pruned20_full.txt", "w") as f:
